		...
		//**************** FIFO **************//	
				if (listePagesSelected1.length <= capacity)
				{
					if (blockNum.indexOf(page1.id)==-1)
					{
						...
						listePagesSelected1[listePagesSelected1.length-1].present=1;
						listePagesSelected1[listePagesSelected1.length-1].reference=1;
...
function recherchIndexPage(listePages,page){
	for (i=0; i<listePages.length; i++) {
		if (listePages[i].id==page.id) return i;
	}
	return -1;
}
function recherchPageProc(page){
 var pageId=page.id;
...
function afficherTab(listePages) {
	arrayDiv.innerHTML = "";
	array1Div.innerHTML = "";
	array2Div.innerHTML = "";
	array3Div.innerHTML = "";
...
function existanceProcessus(numSecteur, numPiste) {
	trouv = false;
	i = 0;
	while ((!trouv) && (i<listeProcessus.length))
	{
...
function ProcessusID(numSecteur, numPiste) {
	trouv = false;
	i = 0;
	while ((!trouv) && (i<listeProcessus.length))
	{
